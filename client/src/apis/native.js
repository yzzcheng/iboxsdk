...
export default {
    ...
    ON_APP_INSTALL: ReactEventListener.ON_APP_INSTALL,
    ON_APP_NOT_FIND: ReactEventListener.ON_APP_NOT_FIND,
    BIND_ACCOUNT:ReactEventListener.BIND_ACCOUNT,
    registry(event, callback) {
        ...
        } else if (event === this.ON_APP_INSTALL) {
            this.onAppInstall = DeviceEventEmitter.addListener(ReactEventListener.ON_APP_INSTALL, (e) => {
                console.log(ReactEventListener.ON_APP_INSTALL, e);
                callback(ReactEventListener, e);
            });
        }  else if (event === this.ON_APP_NOT_FIND) {
            this.onAppNotFind = DeviceEventEmitter.addListener(ReactEventListener.ON_APP_NOT_FIND, (e) => {
                console.log(ReactEventListener.ON_APP_NOT_FIND, e);
                callback(ReactEventListener, e);
            });
        } else if (event === this.BIND_ACCOUNT) {
            this.onBindAccount = DeviceEventEmitter.addListener(ReactEventListener.BIND_ACCOUNT, (e) => {
                console.log(ReactEventListener.BIND_ACCOUNT, e);
                callback(ReactEventListener, e);
            });
        }
    },
    ...
    dispatcherEvent(event, code, isShow, data) {
        ...
        data[this.DIALOG_STATUS] = isShow?1:0;
    ...
    unRegistry() {
        if (event === this.INIT) {
            ...
            this.onAppInstall.remove();
        }else if (event === this.ON_APP_NOT_FIND) {
            this.onAppNotFind.remove();
        }else if(event === this.BIND_ACCOUNT) {
            this.this.onBindAccount.remove();
        }
    }
}